    ...
    if($createpassword !== $confirmpassword){
      ...
      //check if username or email already exist in the database
        ...
        if(mysqli_num_rows($user_check_result) > 0 ){
            ...
            $avatar_destanation_path = 'images/' . $avatar_name;
            ...
            if(in_array($extention, $allowed_files)){
                ...
                if($avatar['size']< 1000000){
                    //upload avatar
                        move_uploaded_file($avatar_tmp_name, $avatar_destanation_path);
                }else {
...
if(isset($_SESSION['signup'])){
  ...
  /*$insert_user_query = "INSERT INTO users (firstname, lastname, username, email , password, avatar , is_admin) values ('$firstname' ,'$lastname' , '$email', '$hashed_password' , '$avatar_name' , 0 ) " ;*/

$insert_user_result = mysqli_query($connection, $insert_user_query);

     if(!mysqli_errno($connection)){
        //redirect to login page with success message
        $_SESSION['signup-success'] = "regestration successful . please log in!";
